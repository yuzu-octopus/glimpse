...
- **Skeletons:** `WidgetChrome` prop `skeletonShape` ('list'|'stat'|'chart'|'rows', default 'rows') from shared `SKELETON_SHAPE` map; `PageSkeleton` is delayed 250ms (`DelayedSkeleton`) so fast loads show nothing. `PageSkeleton` mirrors the ready render's column spans (explicit `span` or `resolveSpan` from `size`) + `spanStyle(span)`, so the skeleton grid matches the real column widths.
